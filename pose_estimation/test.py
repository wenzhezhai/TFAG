...
def get_data_path(root='examples'):
    im_path = [os.path.join(root, i) for i in sorted(os.listdir(root)) if i.endswith('jpg') or i.endswith('jpg')]
    lm_path = [i.replace('jpg', 'txt').replace('jpg', 'txt') for i in im_path]
...
def main(rank, opt, name='examples'):
    ...
    for i in range(len(im_path)):
        print(i, im_path[i])
        img_name = im_path[i].split(os.path.sep)[-1].replace('.jpg','').replace('.jpg','')
        ...
        for r in range(2):
            if r==0:
                ...
                # im_high.save(os.path.join(out_dir_crop1024, img_name+'.jpg'), compress_level=0)
                # im_cropped.save(os.path.join(out_dir_crop1024, img_name+'.jpg'), compress_level=0)
    ...
    with open(os.path.join(name, 'cropping_params.json'), 'w') as outfile:
        print(os.path.join(name, 'cropping_params.json'))
