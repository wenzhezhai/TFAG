...
glob_names= sorted(glob('%s/cropped_images/*.jpg'%(temp_folder)))
...
for name_all in glob_names:
    ...
    pose= np.array(predict_pose[name+'.jpg']['pose']).reshape(16)
    pose_template[:16] = pose

    np.save('%s/'%(output_folder)+name+'.npy',pose_template)
    shutil.copy('%s/cropped_images/'%(temp_folder)+name+'.jpg','%s/'%(output_folder)+name+'.jpg')
    print('%s/'%(output_folder)+name+'.npy')
    print('%s/cropped_images/'%(temp_folder)+name+'.jpg'+'--->'+'%s/'%(output_folder)+name+'.jpg')
