...
# import torchvision.transforms as transforms_plt
...
def generate_images(
    ...
    # Generate images.
    for G, output in zip(G_list, outputs): # ['test_runs/debug/finetuned___ffhq-pixar.pkl']
        for seed_idx, seed in enumerate(seeds):
            ...
            for angle_y, angle_p in [(.4, angle_p), (0, angle_p), (-.4, angle_p)]:
                ...
                if w_pth is not None:
                    ...
                    output_img = output + f'__{w_given_id}.jpg'
                    output_shape = output + f'__{w_given_id}.mrc'
                else:
                    ws = G.mapping(z, conditioning_params, truncation_psi=truncation_psi,  truncation_cutoff=truncation_cutoff)
                    output_img = output + f'__{seed_idx:05d}.jpg'
                    output_shape = output + f'__{seed_idx:05d}.mrc'

                # ws torch.Size([1, 14, 512]) camera_params torch.Size([1, 25])
                img = G.synthesis(ws, camera_params)['image'] # tensor.float32 -1.17-1.09 torch.Size([1, 3, 512, 512]) 
                # img_plt = plt.imread("input_imgs/1/input.jpg") # 0-1 (512, 512, 3) dtype('float32')
                # tensor_plt = transforms_plt.ToTensor()(img_plt) # torch.Size([3, 512, 512]) torch.float32
                # tensor_plt = tensor_plt.unsqueeze(0) # torch.Size([1, 3, 512, 512]) 0-1
                # tensor_plt = (tensor_plt * 255).to(torch.uint8)
                img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8) #torch.Size([1, 512, 512, 3])
                imgs.append(img)
                
                # img torch.Size([1, 512, 512, 3]) # b h w c
            img = torch.cat(imgs, dim=2) # torch.Size([1, 512, 1536, 3])

            # output_img 'test_runs/debug/finetuned___ffhq-pixar.pkl'
            
            PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(output_img)
            
            # print(f"outdir: {outdir}")
            # print(f"output_img: {output_img}")
            # # "../test_runs/test_men/10/manip_3D_recon/4_manip_result"
            # # "../test_runs/test_men/10/manip_3D_recon/4_manip_result/finetuned___ffhq-pixar.pkl__He_is_50_years_old_inv.png"
            
            # img_dir = os.path.join("..", "input_imgs", outdir.split("/")[2], outdir.split("/")[3].split("_")[0])
            # img_name = output_img.split("pkl__")[-1].replace("_inv", "")
            
            # input_image_path = os.path.join(img_dir, img_name)
            # ori_image_path = os.path.join(img_dir, "origin.jpg")
            # ori_mask_path = os.path.join(img_dir, "mask.png")
            
            # print(f"img_name: {img_name}")
            # print(f"input_image_path: {input_image_path}")
            # print(f"ori_image_path: {ori_image_path}")
            # print(f"ori_mask_path: {ori_mask_path}")
            ...
            # generated_image = Image.fromarray(img[0].cpu().numpy(), 'RGB')
            # save_path = os.path.join(outdir, img_name)

            # cat_image(ori_image, ori_mask, input_image, generated_image, save_path)
            ...
            if shapes:
                ...
                head = 0
                with tqdm(total = samples.shape[1]) as pbar: # torch.Size([1, 134217728, 3])
                    with torch.no_grad():
                        while head < samples.shape[1]:
                            torch.manual_seed(0) # torch.Size([1, 1000000, 1])
...
def cat_image(ori_image, ori_mask, input_image, generated_image, save_path):
    w, h = input_image.size
   
    combined_image = Image.new("RGB", (w*6, h))

    combined_image.paste(ori_image, (0, 0))
    combined_image.paste(ori_mask, (w, 0))
    combined_image.paste(input_image, (w * 2, 0))
    combined_image.paste(generated_image, (w * 3, 0))
