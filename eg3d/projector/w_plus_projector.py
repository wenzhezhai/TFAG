...
# freeze_model(vision_model)
# freeze_model(text_model)
...
def project(
    ...
    if (not os.path.exists(w_avg_path)) or (not os.path.exists(w_std_path)):
        ...
        w_samples = G.mapping(torch.from_numpy(
            z_samples).to(device), c_samples)  # [N, L, C]
    ...
    start_w = initial_w if initial_w is not None else w_avg  # [1, 1, 512]

    # Setup noise inputs.
    noise_bufs = {name: buf for (
        name, buf) in G.backbone.synthesis.named_buffers() if 'noise_const' in name}
    ...
    for step in tqdm(range(num_steps), position=0, leave=True):  # 300
        ...
        w_noise = torch.randn_like(w_opt) * w_noise_scale  # [1, 14, 512]
        ws = (w_opt + w_noise)
        synth_images = G.synthesis(ws, c, noise_mode='const')[
            'image']  # [1, 3, 512, 512]

        if step % image_log_step == 0:
            with torch.no_grad():
                vis_img = (synth_images.permute(0, 2, 3, 1) *
...
def project_w_text(
    ...
    for step in tqdm(range(num_steps), position=0, leave=True):  # 300
        ...
        # img text regularization
        text_input = text_tokenizer(text_prompt, padding=True, return_tensors="pt" )
        text_embedding = text_model(**text_input).text_embeds
        synth_images = synth_images.clamp(0, 255).to(torch.uint8)
        vision_input = vision_processor(images=synth_images, return_tensors="pt")
        img_embedding = vision_model(**vision_input).image_embeds # [1, 512]
        dist_mm = (text_embedding - img_embedding).square().sum()
        ...
        loss = dist + reg_loss * regularize_noise_weight + dist_mm * regularize_vl
